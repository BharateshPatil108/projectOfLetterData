...
public class SubjectWiseInnerAdapter extends RecyclerView.Adapter<SubjectWiseInnerAdapter.SubInnerViewHolder> {
    private final Context context;
    private final Map<String, Integer> sub_innerDataList;

    public SubjectWiseInnerAdapter(Map<String, Integer> sub_innerList, Context context) {
        this.sub_innerDataList = sub_innerList;
        this.context = context;
    }

    @NonNull
    @Override
    public SubInnerViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.inner_idev_items, parent, false);
        return new SubInnerViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull SubInnerViewHolder holder, int position) {

        String receivedValue = String.valueOf(sub_innerDataList.get("Received"));
        String disposedValue = String.valueOf(sub_innerDataList.get("Disposed"));
        String acceptedValue = String.valueOf(sub_innerDataList.get("Accepted"));
        String rejectedValue = String.valueOf(sub_innerDataList.get("Rejected"));
        String pendingValue = String.valueOf(sub_innerDataList.get("Pending"));
    ...
    public static class SubInnerViewHolder extends RecyclerView.ViewHolder {
        TextView recived_textView, disposed_textView, accepted_textView, rejected_textView, pending_textView;
        ConstraintLayout constraintLayout;

        public SubInnerViewHolder(@NonNull View itemView) {
            super(itemView);
